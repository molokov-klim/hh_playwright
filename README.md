...
> **Важно**: В headed режиме (HEADLESS=false) браузер будет отображаться только если у вас есть доступ к графическому серверу (X-сервер). В средах без GUI (например, в SSH-сессиях или в облаке) headed режим может не работать корректно.
