...
> **Важно**: В headed режиме (HEADLESS=false) браузер будет отображаться только если у вас есть доступ к графическому серверу (X-сервер). В средах без GUI (например, в SSH-сессиях, в облаке или в CI/CD) headed режим может не работать корректно, и браузер будет запускаться в headless режиме независимо от значения переменной. Это нормальное поведение, так как Playwright не может отобразить окно браузера без графического сервера.
